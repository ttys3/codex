...
The workflow creates a temporary candidate branch and applies only forward changes from the
recorded upstream tag with a three-way merge, judging forward by semantic version rather than by
history. Upstream commits are not added to the candidate history, and `.github/workflows` remains
downstream-owned. This lets the job-scoped GitHub App token push the candidate without permission
to modify Actions workflows. The candidate is then validated and these release packages are built.
The expected workspace-version conflict is resolved deterministically. Up to six remaining text
conflicts can be resolved by GitHub Copilot CLI using the job-scoped `GITHUB_TOKEN` and the
repository owner's Copilot allowance, capped at 30 AI credits per run. Copilot can write only the
original conflict paths; unresolved or invalid output stops the release before a candidate branch
is pushed.
